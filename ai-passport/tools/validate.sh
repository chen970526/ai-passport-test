...
run_static_checks() {
    ...
    # 特斯拉钥匙:业务规则与绑定向导都是纯 C,必须在主机上先测过再谈上板。
    "${CC:-cc}" -std=c11 -Wall -Wextra -Werror -Imain \
        tests/test_tesla_state.c main/tesla_state.c \
        -o "${test_dir}/test_tesla_state"
    "${test_dir}/test_tesla_state"
    "${CC:-cc}" -std=c11 -Wall -Wextra -Werror -Imain \
        tests/test_tesla_wizard.c main/tesla_wizard.c main/tesla_state.c \
        -o "${test_dir}/test_tesla_wizard"
    "${test_dir}/test_tesla_wizard"
    # 控件矩形全部来自 tesla_layout.h,上屏前先在主机证明不越界、不重叠。
    "${CC:-cc}" -std=c11 -Wall -Wextra -Werror -Imain \
        tests/test_tesla_ui_layout.c \
        -o "${test_dir}/test_tesla_ui_layout"
    "${test_dir}/test_tesla_ui_layout"
